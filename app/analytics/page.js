    ...
    <main className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-white transition-colors duration-300">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible" className="mb-10">
          <motion.h2 variants={fadeUp} className="text-3xl font-black">Your Analytics 📊</motion.h2>
          <motion.p variants={fadeUp} className="text-gray-500 dark:text-gray-400 mt-2">Track your interview performance over time.</motion.p>
        </motion.div>
        ...
        <motion.div variants={stagger} initial="hidden" animate="visible" className="grid grid-cols-4 gap-4 mb-10">
          {[
            ...
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-white/5 transition-colors"
            >
          ...
          >
            ...
            <div className="space-y-3">
              {sessions.filter(s => s.score).map((session, i) => (
                ...
                >
                  <span className="text-xs text-gray-400 w-24 shrink-0">{typeLabels[session.type] || session.type}</span>
                  ...
                  <span className="text-sm font-bold text-indigo-500 w-12 text-right">{session.score}/10</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
            ...
            <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-3">
              {sessions.map((session) => (
                <motion.div
                  ...
                  className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-transparent transition-colors cursor-pointer"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-indigo-100 dark:bg-indigo-900 rounded-xl flex items-center justify-center text-lg">
                      {session.type === 'dsa' ? '💻' : session.type === 'behavioral' ? '🧠' : session.type === 'system_design' ? '⚙️' : '🎯'}
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    {session.score ? (
                      <span className="text-indigo-500 font-black text-lg">{session.score}/10</span>
                    ) : (
                      <span className="text-gray-400 text-sm">No score</span>
                    )}
                    <p className="text-xs mt-0.5 capitalize text-gray-400">{session.status}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
